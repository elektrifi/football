...
player_row_df <- data.frame(player_name_v, player_type_v, player_club_v, player_total_points_v, stringsAsFactors = TRUE)
...
library(ggplot2)
#
plot1 <- ggplot(player_row_df, aes(x=player_row_df$player_club_v, y=player_row_df$player_total_points_v, color=player_row_df$player_type_v)) + geom_point(size=3)
# Add a title
...
plot1 <- plot1 + theme(legend.title = element_text(colour="black", size=144, face="bold"))
